...
export function ConversationDisplay({ onStopAudio, viewMode }: ConversationDisplayProps) {
  const { messages, currentTranscript, isAudioPlaying } = useConversationStore();
  const { isLoading, error } = useUIStore();

  const displayMessages = viewMode === 'transcript' ? messages.filter(m => m.isVoice) : messages;
